...
pygame.time.set_timer(VRAGTIMER, 10)
...
class Tile(pygame.sprite.Sprite):
    ...
    def update(self):
        if pygame.sprite.spritecollideany(player, floor_group) != None:
            global col
            if tile_images['paint'] != pygame.sprite.spritecollideany(player, floor_group).image:
                col += 1
...
class Vrag(pygame.sprite.Sprite):
    ...
    def update(self):
        for i in vrag_group:
            if self.tip == 'yellow':
                if self.rect == i.rect:
                    if pygame.sprite.spritecollideany(i, wall_group) != None:
                        self.cuda *= -1
